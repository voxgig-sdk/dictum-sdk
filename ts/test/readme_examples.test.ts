...
//     (\`new DictumSDK(...)\` / \`DictumSDK.test(...)\` become
//     \`DictumSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"author":{"test01":{"id":"test01"}},"category":{"test01":{"id":"test01"}},"quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DictumSDK(...)\` and any
// \`DictumSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DictumSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DictumSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
