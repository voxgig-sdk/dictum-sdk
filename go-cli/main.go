// Command Dictum-cli is a CLI + REPL for the Dictum SDK,
...
const entitiesHelp = "author category quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DICTUM_APIKEY (api key), DICTUM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DICTUM_APIKEY carries the API key and
	// DICTUM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DICTUM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DICTUM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDictumSDK(opts)
