...
boru-driven command-line client **and** interactive REPL for the Dictum
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dictum-cli)
make build
...
./dictum-cli list author
./dictum-cli list category

# 5. Override the API base URL for a single call
DICTUM_BASE=https://api.example.com ./dictum-cli list author
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dictum-cli
   ```
...
Run with no arguments to open a REPL (prompt `dictum>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dictum-cli`. |
...
(`./dictum-cli <expr>`) and interactively (the REPL), and expressions compose the
