...
import (
	...
	omni "github.com/voxgig-sdk/dictum-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DictumError); is {
				out := map[string]any{
					"name":    "DictumError",
...
func omniCtx(arg any, client *sdk.DictumSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DictumSDK); is {
			client = live
