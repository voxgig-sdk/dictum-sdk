...
class DictumMakeError
{
    public static function call(?DictumContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DictumContext([], null);
        }
        $op = $ctx->op ?? new DictumOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DictumResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DictumError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new DictumError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof DictumError) {
