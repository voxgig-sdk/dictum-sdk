...
{
    private static function utility(): DictumUtility
    {
        return new DictumUtility();
    }
    ...
    private static function ctx(array $over = []): DictumContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DictumContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DictumOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DictumError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DictumOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DictumMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DictumOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DictumMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DictumOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DictumMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DictumSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DictumMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DictumResponse([]),
            'result' => new DictumResult([]),
        ]);
        [, $err] = DictumMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DictumSpec(['step' => 's']),
            'response' => new DictumResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DictumResult([]),
        ]);
        [, $err] = DictumMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DictumSpec(['step' => 's']),
            'response' => new DictumResponse(self::resp(200, ['v' => 1])),
            'result' => new DictumResult([]),
        ]);
        [, $err] = DictumMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DictumSpec(['step' => 's']),
            'response' => new DictumResponse(self::resp(200, ['v' => 2])),
            'result' => new DictumResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DictumMakeResponse::call($ctx);
    ...
    {
        $preset = new DictumResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DictumSpec([]),
            'response' => new DictumResponse([]),
            'result' => new DictumResult([]),
        ...
        [$got, $err] = DictumMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DictumResult([])]);
        [, $err] = DictumMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DictumSpec([]), 'result' => null]);
        [, $err] = DictumMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DictumSpec(['step' => 's']),
            'result' => new DictumResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DictumOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DictumMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DictumSpec(['step' => 's']),
            'result' => new DictumResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DictumOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DictumMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DictumResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DictumSpec([]),
            'result' => new DictumResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DictumMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DictumMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DictumError('boom', 'boom');
        $utility->fetcher = function (DictumContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DictumSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DictumMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DictumContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DictumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DictumResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DictumContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DictumSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DictumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DictumResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DictumSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DictumMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DictumMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DictumMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DictumResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DictumDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DictumResult(['ok' => false])]);
        $this->expectException(DictumError::class);
        DictumDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DictumResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DictumMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DictumBaseFeature();
        $b = new DictumBaseFeature();
        DictumFeatureAdd::call($ctx, $a);
        DictumFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DictumBaseFeature
    {
        $f = new DictumBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DictumFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DictumContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DictumSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DictumPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DictumResponse(['headers' => null]),
            'result' => new DictumResult([]),
        ]);
        DictumResultHeaders::call($ctx);
