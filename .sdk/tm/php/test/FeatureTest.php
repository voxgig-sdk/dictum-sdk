...
class FtCtrl extends DictumControl
{
...
class FtClient
{
    ...
    private DictumUtility $utility;

    public function __construct(DictumUtility $utility, array $options)
    {
    ...
    public function get_utility(): DictumUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DictumContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DictumContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DictumConfig::make_feature($name);
    ...
    private static function buildUrl(DictumSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DictumError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DictumResponse($fetched);
        }
    ...
    private function fail(DictumContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DictumContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
