...
class QuoteEntity
  ...
  def data_set(args)
    if args
      @_data = DictumHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DictumHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Quote, Hash] the loaded Quote; raises DictumError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DictumHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Quote>, Array] the matching Quote items; raises DictumError on failure
