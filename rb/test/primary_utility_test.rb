...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = DictumOmni.make_runner(TEST_JSON_FILE, DictumSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : DictumOmni.errify(err)
    end
