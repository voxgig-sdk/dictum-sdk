...
class CategoryEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DICTUM_TEST_CATEGORY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def category_basic_setup(extra)
  ...
  entid_env_raw = ENV["DICTUM_TEST_CATEGORY_ENTID"]
  ...
  if env["DICTUM_TEST_LIVE"] == "TRUE"
    ...
    client = DictumSDK.new(Helpers.to_map(merged_opts))
  end
