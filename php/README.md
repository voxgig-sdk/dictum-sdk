...
The PHP SDK for the Dictum API — an entity-oriented client using PHP conventions.
...
## Install
```bash
composer require voxgig-sdk/dictum
```
...
[$result, $err] = $client->Dictum()->load(["id" => "test01"]);
...
```php
$client = DictumSDK::test($testopts, $sdkopts);
```
...
The main class (`dictum_sdk.php`) exports the SDK class
