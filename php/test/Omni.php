...
//    only - a typed DictumContext there would make every ctx
...
final class DictumOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new DictumContext($clean, null);
    ...
    public static function err_from_map($m): ?DictumError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new DictumError(is_string($code) ? $code : '', $msg);
    }
