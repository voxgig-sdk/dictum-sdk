...
MCP server exposing the Dictum SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `dictum_list` | `entity`, optional `query` map | First page of records as JSON |
| `dictum_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `dictum_list` and
`dictum_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dictum_load","arguments":{"entity":"author","query":{"id":1}}}}'
```
