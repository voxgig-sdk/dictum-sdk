...
[MCP](https://modelcontextprotocol.io) server exposing the Dictum SDK as
two agent tools — `dictum_list` and `dictum_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dictum-mcp)
make build

# 2. Provide credentials via the environment
export DICTUM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dictum \
  -- /absolute/path/to/dictum-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dictum-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dictum_list: first page of records
{ "entity": "author" }
{ "entity": "author", "query": { } }

// dictum_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dictum-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dictum \
     -- "$PWD"/dist/*/dictum-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dictum_list` and `dictum_load` tools now appear
   in new sessions. Ask the agent to *"list author using dictum"*
   and it calls `dictum_list` with `{"entity":"author"}`.
...
```sh
export DICTUM_APIKEY=sk_live_xxx            # API key
export DICTUM_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dictum_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dictum_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DICTUM_APIKEY` | API key sent with every request. |
| `DICTUM_BASE` | Optional override of the API base URL. |
...
```sh
./dictum-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dictum_load","arguments":{"entity":"quote","query":{"id":1}}}}'
```
