    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DICTUM_TEST_CATEGORY_ENTID JSON to run live")
        client = setup["client"]
...
def _category_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("DICTUM_TEST_CATEGORY_ENTID"))
    ...
    if env.get("DICTUM_TEST_LIVE") == "TRUE":
        ...
        client = DictumSDK(helpers.to_map(merged_opts))

    _live = env.get("DICTUM_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DICTUM_TEST_EXPLAIN") == "TRUE",
        "live": _live,
