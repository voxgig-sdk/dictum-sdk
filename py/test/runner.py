...
class DictumTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DictumTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DictumTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DictumTestRunner.getenv("DICTUM_TEST_LIVE")
        override = DictumTestRunner.getenv("DICTUM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DictumTestRunner.getenv(key)
        ...
        explain = DictumTestRunner.getenv("DICTUM_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DICTUM_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DictumTestRunner._test_control is not None:
            return DictumTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DictumTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DictumTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DictumTestRunner.load_test_control()
...
def load_env_local():
    DictumTestRunner.load_env_local()


def env_override(m):
    return DictumTestRunner.env_override(m)


def entity_data(v):
    return DictumTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DictumTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DictumTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DictumTestRunner.load_test_control()


def live_client_options():
    return DictumTestRunner.live_client_options()


def live_delay_ms():
    return DictumTestRunner.live_delay_ms()
